...
/**
 * Mapa temporário: playerId → { waxed: boolean, blockTypeId: string }
 * Armazena o wax_state do bloco quebrado para reaplicar no próximo place.
 */
const pendingWax = new Map();
...
export class CopperChicken {
  static copper_chicken_statue() {
    ...
    // Registra estátuas recém-colocadas e reaplica wax_state se necessário
    world.afterEvents.playerPlaceBlock.subscribe(ev => {
      if (!ALL_STATUE_IDS.has(ev.block.typeId)) return;

      registerStatue(ev.dimension.id, ev.block.location);

      // Reaplica o wax_state do bloco que foi quebrado antes
      const pending = pendingWax.get(ev.player.id);
      if (pending?.waxed && pending.blockTypeId === ev.block.typeId) {
        ev.block.setPermutation(ev.block.permutation.withState('oc:wax', true));
        pendingWax.delete(ev.player.id);
      }
    });

    // Remove do registro e salva wax_state antes de destruir
    world.afterEvents.playerBreakBlock.subscribe(ev => {
      if (!ALL_STATUE_IDS.has(ev.brokenBlockPermutation.type.id)) return;

      unregisterStatue(ev.dimension.id, ev.block.location);

      const waxed = ev.brokenBlockPermutation.getState('oc:wax');
      if (waxed) {
        // Guarda o estado por 200 ticks (10s) — tempo suficiente para recolocar
        pendingWax.set(ev.player.id, {
          waxed: true,
          blockTypeId: ev.brokenBlockPermutation.type.id,
        });
        system.runTimeout(() => pendingWax.delete(ev.player.id), 200);
      }
    });
